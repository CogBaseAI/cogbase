...
The workflow commands require persistent store backends. The Docker demo image ships with SQLite + FAISS + local file storage, so they work out of the box; pass a host data directory to `docker_hub_demo.sh run` to persist data across container restarts (see `server/README.md`).
