...
def build_structured_store(cfg: StructuredStoreConfig) -> StructuredStoreBase:
...
def build_vector_store(cfg: VectorStoreConfig) -> VectorStoreBase:
    """Instantiate a vector store from config."""
    if cfg.type == "faiss":
        from cogbase.stores.vector.faiss_store import FAISSVectorStore
        return FAISSVectorStore(path=cfg.path)
...
def build_document_store(cfg: DocumentStoreConfig) -> DocumentStoreBase:
    """Instantiate a document store from config."""
    if cfg.type == "local":
        from cogbase.stores.document.local_fs import LocalFSDocumentStore
        return LocalFSDocumentStore(cfg.path)
    if cfg.type == "s3":
        from cogbase.stores.document.s3 import S3DocumentStore
        return S3DocumentStore(bucket=cfg.bucket, prefix=cfg.prefix, region=cfg.region)
