...
class TestDocumentEmbedUpsert:
    ...
    async def test_llm_failure_does_not_raise(self, make_vector_store):
        ...
        count, chunks_written, extraction_failed, _, _ = await pipeline._ingest(Document(doc_id="d-001", text="text"))
        assert count == 0
...
class TestThreeStepPipeline:
    ...
    async def test_all_three_steps_execute(self, make_vector_store, make_structured_store):
        ...
        count, chunks_written, extraction_failed, _, _ = await pipeline._ingest(Document(doc_id="d-001", text="word " * 20))
        ...
        # chunk-embed-upsert chunks + the one document-embed-upsert summary chunk.
        assert chunks_written == chunk_store.ntotal("chunks") + 1
        assert not extraction_failed
