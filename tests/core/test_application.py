...
class TestIngestionPipelineIngest:
    ...
    async def test_ingest_returns_record_count(self):
        ...
        count, chunks_written, extraction_failed, _, _ = await app._ingest(Document(doc_id="doc-1", text="hello world contract clause"))
        assert count == 1
    ...
    async def test_ingest_empty_text_returns_zero(self):
        app, vector_store, _ = await self._make_app()
        count, chunks_written, extraction_failed, _, _ = await app._ingest(Document(doc_id="doc-empty", text=""))
        assert count == 0
        assert chunks_written == 0
        assert vector_store.ntotal("docs") == 0
    ...
    async def test_vector_only_app_ingest_returns_zero(self):
        ...
        count, chunks_written, extraction_failed, _, _ = await app._ingest(Document(doc_id="doc-1", text="word " * 30))
        assert vector_store.ntotal("docs") > 0
