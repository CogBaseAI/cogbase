...
Every document is processed into structured facts at ingestion time. Each fact carries: `type`, `value`, `raw_text`, `doc_id`, `page`, `confidence`. The `raw_text` field is what gets shown to users as the citation — it's preserved verbatim from the source.
